...
class PackCirclesImage:
    ...
    def read_image(self):
        self.REF_IMAGE = cv2.imread(self.IMAGE_PATH)
        self.REF_IMAGE = cv2.resize(self.REF_IMAGE, (self.WIDTH+1, self.HEIGHT+1))
        self.REF_IMAGE = self.REF_IMAGE.reshape(self.HEIGHT+1, self.WIDTH+1, 3).astype("uint8")
    ...
    def show_image(self):
        cv2.imshow("PackCirclesImage", self.image)
        key = cv2.waitKey(0)
        if key == ord("s"):
            cv2.imwrite("PackCirclesImage.png", self.image)
...
rc = PackCirclesImage(640, 427, "docs/test_image.jpg", MAX_RADIUS=5, MIN_RADIUS=3, AMOUNT=2500)
rc()